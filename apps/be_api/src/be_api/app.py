...
app = FastAPI(title="IKG Hybrid Search Gateway", version="0.7.0")
...
# SQLite I/O 잠금 경합 소거를 위한 전역 비동기 상호 배제 뮤텍스 가드
...
async def create_bookmark_endpoint(payload: BookmarkIngestRequest):
    ...
    # 💡 [FIXED]: asyncio.run()을 걷어내고 세마포어가 통제하는 인스턴스를 await로 직렬 전개
    scraped_title, scraped_content = await scrape_url_standalone(target_url)
    
    # 실시간 수집 결과에 대한 정밀 교차 검증 집행
    ...
    # 💡 [MUTEX LOCK]: 비동기 트래픽의 동시 커밋 잠금 경합을 하드웨어 레이어에서 전격 차단
