...
def search_bookmarks_endpoint(q: Optional[str] = None, query: Optional[str] = None, limit: int = 5):
    ...
    logger.info(f"[API GET SEARCH] 하이브리드 검색 처리량 인입 -> 쿼리: '{effective_query}' | Limit: {limit}")
    
    if not effective_query or not effective_query.strip():
        return []
        
    try:
        # [CQRS 가드] 동기식 다중 사용자 조회를 위해 락 경합 없이 읽기 파이프라인 관통
...
def get_similarity_graph_topology(threshold: float = 0.85):
    ...
    try:
        ...
        # [CRITICAL BUG FIX]: IndexIDMap 체제에 입각해 순차 인덱스 i가 아닌 실제 도큐먼트의 고유 정수 PK를 넘겨 공간 복원하도록 전면 정정
...
def update_bookmark_endpoint(bookmark_id: int, payload: BookmarkCreateRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not cursor.fetchone():
            raise HTTPException(status_code=404, detail="수정 복구하려는 대상 기술 자산이 존재하지 않습니다.")
        ...
        # 중복 방지 선제 소거 파이프라인 및 재임베딩 유도를 위해 가상 큐 재주입
        background_tasks.add_task(embedded_task_queue.put, bookmark_id)
        
        return TaskReceiptResponse(
            message="북마크 메타데이터 수정이 커밋되었습니다. 벡터 재인덱싱이 시작됩니다.",
...
def delete_bookmark_endpoint(bookmark_id: int):
    """물리 디스크 I/O 유발을 지연 격리시켜 수 밀리초 이내 레이턴시를 사수하는 영리한 고속 삭제"""
    ...
    try:
        ...
        # [중요] 사용자가 체감하는 즉시 정합성(Immediate Consistency) 사수를 위해 인메모리 맵 스냅샷 즉시 리프레시
        searcher_engine.refresh_context()
        logger.info(f"[API DELETE SUCCESS] #{bookmark_id} 논리 소거 완료 및 조회 풀 강제 최신화 완결.")
...
def force_refresh_infrastructure_context():
    """indexing.py 벌크 임포터 기동 직후 메인 서버 콘텍스트 가상 스냅샷을 즉시 동기화 복원하는 라우터"""
    logger.info("[API SYSTEM] 외부 요인 대량 적재에 따른 인메모리 캐시 강제 무효화 및 동기화 쿼리 수신")
    try:
        searcher_engine.refresh_context()
        return {"status": "SUCCESS", "message": "전역 가상 메모리 스냅샷 및 BM25 렉시컬 인덱스 동기화 완결."}
    except Exception as e:
        logger.critical(f"시스템 동기화 복원 실패: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"인프라 컨텍스트 수렴 오류: {str(e)}")
